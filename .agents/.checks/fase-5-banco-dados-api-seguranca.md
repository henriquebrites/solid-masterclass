...
| `buildApp` aceita um segundo parâmetro opcional `usersRateLimit` (default `{ max: 5, timeWindow: "1 minute" }`) além de `FastifyServerOptions`, usado só para configurar o rate limit da rota `POST /users` | `buildApp({ usersRateLimit }: { usersRateLimit?: ... } & FastifyServerOptions = {})`; `index.ts` continua chamando `buildApp()` sem argumentos, herdando o default de produção (5/min) | manter o limite fixo em 5/min sem seam de teste - rejeitado porque `app.test.ts` usa uma única instância `app` compartilhada entre ~25 requisições `POST /users` no `beforeAll`; sob o limite real de produção, a maioria dessas chamadas passaria a receber 429 e quebraria testes já existentes e aprovados. O teste do limite real (429 na 6ª requisição) usa uma instância própria, construída com o default de produção |
...
- **Rate limit por rota (5.5)**: decisão binária (excedeu 5/min ou não) provada no boundary HTTP, único lugar onde ela é observável. O `setErrorHandler` (5.7) generalizou o repasse de `error.statusCode` (em vez de checar só `error.validation`) para cobrir tanto erros de validação zod (400) quanto o erro do `@fastify/rate-limit` (429) com o mesmo mecanismo, sem um `if` por plugin.
